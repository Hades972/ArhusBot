...
client.login("NDY4NTM4NjU2Mjg2NzAzNjQy.DjT19A.pBpsBJ_wcX8XBWYu_DC-1miFEFs");

client.on("ready", () => {

    console.log("Je suis prêt !");
    client.user.setActivity(("Arhus✭"));
...
client.on('message', async message => { 
    ...
    if(message.content === prefix + "mod") {
      ...
      .setColor('RANDOM')
      .setTitle(`:tools: Voici les commandes de modérations du bot !`)
      .setThumbnail(message.author.avatarURL)
      .addField("*kick <@user>", "Kick un utilisateur !")
      .addField("*ban <@user>", "Bannir un utilisateur !")
      .addField("*clear nombre", "Supprime un nombre de messages indiqué !")
      .addField("*mute <@user>", "Mais l'utilisateur en sourdine !")
      .addField("*unmute <@user>", "Unmute l'utilisateur qui a été mute !")
      .setFooter("Commande modération - Ahrus")
      .setTimestamp()
    ...
    if(message.content === prefix + "fun") {
      ...
      .setFooter("Commande modération - Arhus")
      .setTimestamp()
    ...
    if(message.content.startsWith(prefix + "kick")){
        ...
        if(message.mentions.users.size === 0) {
            return message.channel.send(":x:Vous devez mentionner un utilisaeur")
        }
        var kick = message.guild.member(message.mentions.users.first());
        if(!kick) {
            return message.channel.send(":sweat_smile:Je ne sais pas si l'utilisateur existe :/")
        }
    
        if(message.guild.member(client.user).hasPermission("KICK_MEMBERS")) {
            return message.channel.send(":gear: Je n'ai pas la permission pour kick");
        }
    
        kick.kick().then(member => {
            message.channel.send(`:white_check_mark: ${member.user.username} a été kick par ${message.author.username}`);
        });
    }
    ...
    if(message.content.startsWith(prefix + "ban")) {
        ...
        if(!message.guild.member(client.user).hasPermission("BAN_MEMBERS")) {
            return message.channel.send("Je n'ai pas la permission pour bannir cet utilisateur");
        }
        ban.ban().then(member => {
            message.channel.send(`:white_check_mark: ${member.user.username} a été ban par ${message.author.username} !`)
        });
    ...
    if(message.content.startsWith(prefix + "clear")) {
        ...
        if(!args[0]) return message.channel.send(":x: Vous devez préciser un nombre de messages à supprimer !")
        message.channel.bulkDelete(args[0]).then(() => {
            message.channel.send(`:white_check_mark: ${args[0]} messages ont été supprimés avec succès !`);
        });
    }

    if(message.content.startsWith(prefix + "mute")) {
        if(!message.guild.member(message.author).hasPermission("ADMINISTRATOR")) return message.channel.send(":x: Vous n'avez pas la permission de ban !");
        ...
        message.channel.overwritePermissions(mute, { SEND_MESSAGES: false}).then(member => {
            message.channel.send(`:white_check_mark: ${mute.user.username} est maintenant mute !`);
        });
    }
    ...
    if(message.content.startsWith(prefix + "unmute")) {
        ...
        message.channel.overwritePermissions(mute, { SEND_MESSAGES: true}).then(member => {
            message.channel.send(`${mute.user.username} est maintenant unmute !`);
        });
    }
